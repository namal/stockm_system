...
<div class="container-responsive mx-0"> <!--d1-->
  <h3 class="m-0 pt-0 mb-3 w3-animate-left">
    ...
    <span class="">Bin Allocation for Received Materials</span>
  </h3>
  <div class="bg-light">
    <div class="row">
    ...
    <div class="row">
      <div class="input-group mb-3">
        ...
        <a class="btn btn-sm btn-info mx-3" type="button" style="color:white; width:100px;" href="/print-receive-bin-allocate_all">
    ...
    <div class="" id="customers">
      <table class="table table-responsive table-sm caption-top table-hover table-bordered text-center" id="myreceiveAllocationTable">      
        ...
        <tbody>
          ...
          <tr class="bg-white">
            ...
            <th scope="row">{{ $i++ }}</th>
            {{-- <th scope="row">{{ $receive->firstItem()+$loop->index }}</th> --}}
            ...
            <td>           
              ...
              <form method="POST" id="delete-bin-{{ $receive->id }}" action="{{ route('delete_receive_bin_allocation', $receive->id) }}" style="display: none;">
                {{ csrf_field() }}
                {{ method_field('delete') }}
              </form>  
              <button onclick="if(confirm('Are you sure to delete data?')){
                event.preventDefault();
                document.getElementById('delete-bin-{{ $receive->id }}').submit();
                }else{
                  event.preventDefault();
                }"
